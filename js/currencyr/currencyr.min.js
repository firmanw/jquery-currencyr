...
b("body").data("currencyr_data",e);};var a=function(d,c){this.target=d;this.options=b.extend({},this.defaults,c);this.init();return this;};a.prototype={target:null,defaults:{numcodes:5,pad:20,root:"currencyr"},data:{base:"",currencies:{},rates:{}},current:null,amount:0,classes:{container:"container","close-button":"close",converter:"converter",amount:"amount",code:"code",currency:"currency",codes:"codes",dropdown:"dropdown",target:"target"},elements:{},dimensions:{main:0,dropdown:0,total:0},direction:"bottom",init:function(){if(!this.prepare()){return;
...
});if(window.addEventListener){c.elements.dropdown[0].addEventListener("DOMMouseScroll",c.dropdownWheel,false);c.elements.dropdown[0].addEventListener("mousewheel",c.dropdownWheel,false);
}b('input[type="checkbox"]',c.elements.remember).unbind().bind("click",function(){c.remember(b(this).attr("checked"),c.elements.currency.text());});},show:function(h){var k=this,e=k.data.base,g=0,f=b(h).offset(),j=b(h).height(),d=k.viewport(),l=k.elements.beak.height()+2,c=f.top+j+l,i=c+k.options.pad;
...
if(b.cookie("currencyr")!=null){e=b.cookie("currencyr");b('input[type="checkbox"]',k.elements.remember).attr("checked",true);}k.elements.currency.text(e);
k.elements.code.removeClass(this.cssClass("dropped"));k.amount=accounting.unformat(b(h).text());g=e!=k.data.base?accounting.formatMoney(k.convert(e),""):k.amount;
...
},dropdownOut:function(c){this.elements.dropdown.width(this.elements["codes-list"].outerWidth());},dropdownWheel:function(h){var g=h||window.event,i=g.wheelDelta?g.wheelDelta/120:-g.detail,c=b(this).data("scroll")||0,f=b("ul",this).height(),d=b(this).height();
c-=Math.ceil(i)*b("ul li",this).first().outerHeight();c=Math.min(f-d,Math.max(0,c));b(this).data("scroll",c);b("ul",this).css("top",-c);g=b.event.fix(g);
